...
(function(){
 ...
 function rs(d){
  ...
  const cards=leaders.map((r,i)=>kpi(`${i?'US':'KR'} 상대강도 1위`,r?.name||'산출 불가',r?`z ${F(r.z,'σ',2)} · ${r.zone}`:'가격 확인 필요')).join('')+kpi('최약세 · 전체',weak?.name||'산출 불가',weak?`z ${F(weak.z,'σ',2)} · ${weak.zone}`:'')+kpi('주도주 1위 · 오닐 RS','유니버스 확인 필요','원본의 전체 대상 종목이 미공개');
  ...
  return header(d,'주간 상대강도 (RS)')+`<div class="kpi-grid">${cards}</div><div class="coverage-note">페어 ${d.coverage.pairs}/${d.coverage.expected_pairs} 산출 · 확인되지 않은 페어도 목록에 표시합니다.</div>`+controls('rs',[['all','전체'],['KR','한국'],['US','미국']],'market')+
   `<div class="chart-grid summary-charts">${chartCard('섹터·스타일 z-score 상위 14',C.bars(rows.filter(valid).slice(0,14).map(r=>({name:r.name,value:r.z})),{unit:'σ',markers:[-2.58,-1,1,2.58],title:'상대강도 순위'}),'3M 기간수익률 스프레드의 5Y z-score')}${chartCard('KR 대형주 · 1M 수익률 Top / Worst',C.line({reason:d.stock_rankings.reason}),'원본 구조: 1W로 선별한 강세 8·약세 6종목의 1M 수익률 막대')}${chartCard('US 대형주 · 1M 수익률 Top / Worst',C.line({reason:d.stock_rankings.reason}),'원본 구조: 1W로 선별한 강세 8·약세 6종목의 1M 수익률 막대')}</div>`+
