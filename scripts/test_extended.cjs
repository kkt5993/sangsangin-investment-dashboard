...
for(const file of ['charts','analysis-charts','network-views','relation-views','decision-ledger','research-store','research-notes','geo-views','earnings-views','ownership-views','calendar-views','research-dashboard'])vm.runInContext(read('docs/'+file+'.js'),context);
...
(async()=>{
 await relationChecks;await overviewChecks;await geoChecks;await earningsChecks;await ownershipChecks;await calendarChecks;await researchChecks;
 let count=0;
