...
(function(root){
 ...
 function surface(c,yaw=-.65,count=null,zoom=1,tilt=.48){
  ...
  if(finite(surf[nt-1][nw-1])){const current=P(nw-1,nt-1,surf[nt-1][nw-1]),floor=P(nw-1,nt-1,lo);b+=`<line x1="${floor[0]}" y1="${floor[1]}" x2="${current[0]}" y2="${current[1]}" stroke="#ba812d" stroke-width="2"/><circle data-surface-marker="${E(c.dates[nt-1])}" cx="${current[0]}" cy="${current[1]}" r="5" fill="#c08a32"/><text x="${current[0]+10}" y="${current[1]-8}" class="axis">${nt===c.dates.length?'최신':'선택 시점'}</text>`;}
