...
(function(root){
 ...
 function bars(items,{unit='',markers=[],title='',digits=1,valueUnit=unit,signalMode=false,extentPadding=1.1}={}){
  ...
  const extent=Math.max(1,...rows.map(r=>Math.abs(r.value)),...markers.map(Math.abs))*extentPadding;
  ...
  const body=rows.map((r,i)=>{const y=T+i*rowH,end=X(r.value),signal={long:'상승 추세',short:'하락 추세',neutral:'중립',missing:'미산출'}[r.signal]||'미산출',color=signalMode?({long:'#288376',short:'#ab5776',neutral:'#b58a3f'}[r.signal]||'#7d8c99'):(r.value>=0?'#288376':'#9a6da2');return `<g><title>${esc(r.name)}: ${fmt(r.value,unit,2)}${signalMode?' · '+signal:''}</title><text x="${L-10}" y="${y+18}" text-anchor="end" class="bar-name">${esc(r.name)}</text><rect data-bar-value="${r.value}" x="${Math.min(zero,end)}" y="${y+3}" width="${Math.max(.8,Math.abs(end-zero))}" height="23" fill="${color}" data-bar-signal="${signalMode?esc(r.signal||'missing'):'none'}"/><text x="${end+(r.value>=0?7:-7)}" y="${y+19}" text-anchor="${r.value>=0?'start':'end'}" class="bar-value">${fmt(r.value,valueUnit,digits)}</text></g>`;}).join('');
