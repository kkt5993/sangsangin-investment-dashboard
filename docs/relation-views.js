...
(function(root){
 ...
 function graphSVG(g,st={}){
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 560" class="relation-svg" role="img" aria-label="사업 관계망 3차원 투영. 객체 선택, 드래그 회전, 휠 확대."><rect width="900" height="560" rx="14" fill="#101d2b"/>${edges.map(edge).join('')}${[...nodes].sort((a,b)=>pos.get(b.id).z-pos.get(a.id).z).map(n=>{const p=pos.get(n.id),label=labels.get(n.id),on=n.id===selected;return `<g data-rnode="${E(n.id)}" tabindex="0" role="button" aria-label="${E(n.name)} 관계 선택" transform="translate(${p.x.toFixed(2)} ${p.y.toFixed(2)})" opacity="${!selected||on||neighbors.has(n.id)?1:.56}"><circle r="${p.r.toFixed(2)}" fill="${colors[n.kind]}" stroke="${on?'#fff':'#172939'}" stroke-width="${on?3:1}"/><text x="${(label.x-p.x).toFixed(2)}" y="${(label.y-p.y).toFixed(2)}" text-anchor="middle" fill="#e0eaf4" font-size="11">${E(label.text)}</text><title>${E(n.role)}${n.date?' · '+E(n.date):''}</title></g>`;}).join('')}<text x="18" y="540" fill="#b6c5d1" font-size="12">표시 ${nodes.length}객체 · ${edges.length}관계 · 점선: 가격 상관 · 드래그 회전 / 휠 확대</text></svg>`;
 }
 ...
 function bind(container,d,navigate,selection={}){
  ...
  container.querySelectorAll('[data-relation-view]').forEach(box=>{
   ...
   if(mode==='relationlab')body.innerHTML=`<div class="relation-hubs"><strong>시스템 중심성 · 기본3단계</strong>${g.hubs.filter(h=>h.score>0).slice(0,6).map(h=>`<button data-rhub="${E(h.id)}">${E(name(g,h.id))} ${N(h.score)} · ${h.breadth}객체</button>`).join('')}</div><p class="quiet">점수=도달 |강도| 합+0.15×객체 수 (|강도|≥0.05). 모형 연결성 순위입니다. 상관 전파 선택과 무관한 기본값입니다.</p><div class="analysis-controls">${select('node','객체',g.nodes.map(n=>[n.id,n.name]),st.node)}<label>단일 충격 <input data-rshock type="number" min="-2" max="2" step="0.1" value="${st.shock}"></label><label>확대 <input data-rzoom type="range" min="50" max="240" value="${st.zoom*100}"></label><label>크기 <input data-rsize type="range" min="60" max="180" value="${st.size*100}"></label><button data-rspin>자동 회전 시작</button><button data-rreset>시점 초기화</button></div><p class="quiet">종류·관계·검색은 지도 표시 필터입니다. 충격 계산은 전체 근거 관계망에서 수행합니다.</p><div data-relation-graph></div><div data-relation-detail></div><div data-relation-output></div>`;
   ...
   if(graph){const contacts=new Map();let pinch=null;const distance=()=>{const [a,b]=[...contacts.values()];return a&&b?Math.hypot(a.x-b.x,a.y-b.y):null;};
    graph.addEventListener('pointerdown',e=>{stop();drag={x:e.clientX,y:e.clientY};contacts.set(e.pointerId,drag);pinch=distance();dragged=false;});
    graph.addEventListener('pointermove',e=>{if(!drag)return;contacts.set(e.pointerId,{x:e.clientX,y:e.clientY});const current=distance();if(current!==null&&pinch){dragged=true;graph.setPointerCapture?.(e.pointerId);st.zoom=Math.max(.5,Math.min(2.4,st.zoom*current/pinch));pinch=current;box.querySelector('[data-rzoom]').value=String(st.zoom*100);draw();return;}
     const dx=e.clientX-drag.x,dy=e.clientY-drag.y;if(Math.abs(dx)+Math.abs(dy)<=2)return;dragged=true;graph.setPointerCapture?.(e.pointerId);st.yaw+=dx*.008;st.pitch=Math.max(-1.4,Math.min(1.4,st.pitch+dy*.008));drag={x:e.clientX,y:e.clientY};draw();});
    const end=e=>{contacts.delete(e.pointerId);drag=[...contacts.values()][0]||null;pinch=distance();};graph.addEventListener('pointerup',end);graph.addEventListener('pointercancel',end);
    graph.addEventListener('wheel',e=>{e.preventDefault();st.zoom=Math.max(.5,Math.min(2.4,st.zoom*Math.exp(-e.deltaY*.001)));box.querySelector('[data-rzoom]').value=String(st.zoom*100);draw();},{passive:false});
   }
   disposers.push(()=>{disposed=true;stop();});draw();detail();update();
  });
 }
