...
교역은 `pipeline.trade_data`에서 실행 연도보다2년 전의 상품 총수출을45개 통계지역별로 확인한다. 30일 이내 지역 캐시와180일 코드 사전을 재사용하며, 같은 값은 원자료를 복제하지 않고 확인 시각만 원장에 남긴다. 1MiB/응답·500행 검사·직렬2초 간격이며401/403/429는 해당 실행의 후속 요청을 중단한다. 실패 또는 기존 자료의 갑작스러운 소실은 이전 관측·성공시각을 유지한다. [교역 데이터 계약](TRADE_CONTRACT.md)을 따른다.

위성은7일마다 검증된 시설 목록의 최근60일·최대12후보를 확인합니다. HTTP Range만 허용하고1회수집192MiB전송을 제한합니다. 같은촬영 장면은 이전 원자료를 상속하고, 실패/구름이 많으면 이전 관측 날짜를 유지하며 상태를 표시합니다. 위치 검색은 정기 실행에 넣지 않습니다. 공개 파일은400×400 RGB/NDVI PNG이며 원밴드는PC에 보관합니다.
