"""SAURON public scene: reviewed facilities, observed events, model orbit elements."""
...
def build(d):
    ...
    return dict(type='sauron',group='SAURON',title='SAURON · 3D 지구 관측',
...
def views(d,obj):
    obj['sections']=[s for s in obj['sections'] if s.get('group')!='SAURON'];obj['sections'].append(build(d));obj['generated_at']=stamp()
    obj['missing']=[m for m in obj['missing'] if not m.startswith('SAURON의')]
    obj['missing'].append('SAURON의 Google 실사3D는 별도 키가 필요합니다. Wikipedia 지명은 문서 대표 좌표이며 주소 검색과 다릅니다. 지진은 PC 수집 시점의 관측이고 위성 위치는 궤도 모델 계산입니다.')
    return obj
