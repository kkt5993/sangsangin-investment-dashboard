...
(function(){
 ...
 function rs(d){
  ...
  const stock=d.stock_rankings?.KR?.leaders?.[0];
  const cards=leaders.map((r,i)=>kpi(`${i?'US':'KR'} 상대강도 1위`,r?.name||'산출 불가',r?`z ${F(r.z,'σ',2)} · ${r.zone}`:'가격 확인 필요')).join('')+kpi('최약세 · 전체',weak?.name||'산출 불가',weak?`z ${F(weak.z,'σ',2)} · ${weak.zone}`:'')+kpi('KR 대형주 · 오닐 RS 1위',stock?.name||'산출 대기',stock?`RS ${F(stock.rs,'',0)} · ${stock.sector}`:'KRX 공식 대형주 기준');
  ...
  return header(d,'주간 상대강도 (RS)')+`<div class="kpi-grid">${cards}</div><div class="coverage-note">페어 ${d.coverage.pairs}/${d.coverage.expected_pairs} 산출 · 확인되지 않은 페어도 목록에 표시합니다.</div>`+controls('rs',[['all','전체'],['KR','한국'],['US','미국']],'market')+
   `<div class="chart-grid summary-charts">${chartCard('섹터·스타일 z-score 상위 14',C.bars(rows.filter(valid).slice(0,14).map(r=>({name:r.name,value:r.z})),{unit:'σ',markers:[-2.58,-1,1,2.58],title:'상대강도 순위'}),'3M 기간수익률 스프레드의 5Y z-score')}${['KR','US'].map(m=>{const r=d.stock_rankings[m];return chartCard(m+' 대형주 · 1M 수익률 Top / Worst',r?C.bars([...r.strong,...r.weak].map(a=>({name:a.name,value:a.r1m})),{unit:'%',title:m+' 대형주'}):C.line({reason:'가격 수집 대기'}),'1W로 선별한 강세 8·약세 6종목의 1M 수익률');}).join('')}</div>`+stockTables(d.stock_rankings)+
   heading('상대강도 히트맵','열별 색상 스케일 · 숫자와 단위를 함께 비교')+C.heatmap(hm,['3M 차이 (%p)','z (σ)','Δz 1W (σ)'],'한국·미국 섹터와 스타일')+
   heading('페어별 상세 수치')+pairTable(rows)+heading('5년 상대강도 추이','0 · ±1 · ±2σ 기준선')+
 ...
 function momentum(d){
  ...
  const barGroups=['country','factor','asset'].filter(g=>sel==='all'||g===sel);
  let charts=barGroups.map(g=>{const rows=d.assets.filter(a=>a.group===g).sort((a,b)=>(b.returns['3M']??-Infinity)-(a.returns['3M']??-Infinity)).slice(0,g==='country'?12:99);return chartCard(`${groupNames[g]} · 3M 수익률${g==='country'?' 상위 12':''}`,C.bars(rows.map(a=>({name:`${a.name} (${a.symbol})`,value:a.returns['3M']})),{unit:'%',title:groupNames[g]}),`공통 기준일 ${d.asset_as_of} · USD 표시 ETF / BTC`);}).join('');
  if(sel==='all'||sel==='sector')charts+=chartCard('섹터 로테이션 · 3M 상대강도 z',C.bars(sectors.map(r=>({name:r.name,value:r.z})),{unit:'σ',markers:[-2.58,-1,1,2.58],title:'섹터 로테이션'}),`원본 24개 중 ${d.coverage.sectors}개 산출 · 기준일 ${d.as_of}`);
  ...
  if(sel==='all'||sel==='sector'){
   html+=heading('섹터별 상대강도')+pairTable(sectors)+heading('섹터별 누적 초과수익','상위 16개 · 왼쪽 3M / 오른쪽 6M')+`<div class="chart-grid">${d.chart_pairs.map(id=>{const r=sectors.find(x=>x.id===id);return [3,6].map(n=>{const c=d.curves[id]?.[String(n)];return chartCard(`${r.name} · ${n}M · 누적초과 ${F(c?.last,'%')}`,C.line(c,{kind:'momentum',title:`${r.name} ${n}M`}),`${r.a} / ${r.b} · 가격비의 시작점 대비 변화율 · 0 기준 양/음 영역`);}).join('');}).join('')}</div>`;
  }
