...
핵심4개는 구리선물/금선물, SPY/TLT, XLY/XLP, EPU·VIX·HY 동일가중 z다. 앞3개는 비조정 종가의 공통 날짜 비율이며180공통관측 곡선과63공통관측 변화율을 제공한다. 마지막은 지정학 탭의 기존 복합을 그대로 참조하며 변화는 z 차이다. z값의 퍼센트 변화로 혼동하지 않는다. 변환 방식·단위가 다른 수치를 하나의 점수로 합산하지 않는다.
