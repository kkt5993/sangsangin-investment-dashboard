...
def principium(as_of):
    return module('principium',as_of,'논문·리포트·프라이머를 제목/메타·핵심·아이디어·근거·시사점의5단으로 열람하고 등록·수정합니다. 제목 노드의 공유 키워드 관계와 빈도 구체를 문서에서 계산합니다. PDF 파서를 우선 사용하고 필요한 스캔 페이지만 한글·영문 OCR로 읽습니다. 제목·저자·페이지 본문·원문 해시를 근거에 연결하며 처리 방식·시간을 보존합니다. 초기 공개 콘텐츠는 팀 구현 노트이며 새 글·첨부·추출 본문은 이 브라우저의 로컬 자료입니다.',
        [dict(type='notebook',title='리서치 아카이브',group='리서치 아카이브',mode='principium',items=LIBRARY)],missing=['PDF 텍스트 추출은 파일당25MiB·300쪽·20만자 범위입니다. OCR은 회당30쪽·최대3분이며 완료 페이지는 재사용합니다. 숫자·표·복잡한 배치는 원문 대조가 필요하고 LLM 요약·번역은 추가 구현 대상입니다.','이 브라우저의 IndexedDB에 저장합니다. 팀 공용 DB·서버 인증/공개 배포·서버 조회 통계는 추가 구현 대상입니다.'])
def libraries(d):
