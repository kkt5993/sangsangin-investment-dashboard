...
(function(root){
 ...
 const kinds={all:'전체',linked:'객체 연결',official:'공식 근거',team:'팀 방법론',local:'로컬 PRINCIPIUM'};
 ...
 function bind(container,snapshot,navigate){container.querySelectorAll('[data-dr-research]').forEach(box=>{
  ...
  box.addEventListener('click',async event=>{const entity=event.target.closest('[data-dr-entity]');if(entity){navigate?.('Entity 360',{entity:entity.dataset.drEntity});return;}const file=event.target.closest('[data-dr-file]');if(!file||file.disabled)return;file.disabled=true;let repo,object;
