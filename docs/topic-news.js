...
(function(root){
 ...
 function detail(topic,provider,section){
  const deferred=topic.error==='collection_aborted',reason=deferred?topic.abort_reason:topic.error,http=deferred?topic.abort_http_status:topic.http_status;
  const reasons={HTTPError:'제공처 HTTP 오류',ReadTimeout:'응답 시간 초과',ConnectTimeout:'연결 시간 초과',ConnectionError:'연결 실패'};
  const collectionState=deferred?'배치 중단으로 조회 보류':(topic.error?(reasons[topic.error]||topic.error):(topic.retrieved_at?'성공 조회 자료 보존':'미조회'));
  const cause=reason?`${reasons[reason]||reason}${http?' · HTTP '+http:''}`:'';
  const collectionInfo=`<p class="quiet">성공 조회 ${E(topic.retrieved_at||'미확보')} · 최근 실제 시도 ${E(topic.checked_at||'미확보')}<br>수집 상태 ${E(collectionState)}${deferred?'<br>보류 시각 '+E(topic.deferred_at||'미확보'):''}${cause?'<br>원인 '+E(cause):''} · 실패 시 이전 자료를 보존합니다.</p>`;
