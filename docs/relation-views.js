...
(function(root){
 ...
 function universeHTML(g){
  const c=g.universe_coverage;if(!c)return '';
  return `<details data-relation-universe><summary>기업 범위 · ${c.companies.length}기업 / ${c.mapped_aliases}연구표기 연결 · 미연결 ${c.unresolved.length}표기</summary><p>${E(c.note)}</p><p>관찰 대상에 포함해도 사업 연결이 없을 수 있습니다. 가격 상관선은 사업 공급관계와 구분합니다. 기업명은 기존 공식 종목 자료를 사용합니다.</p><div class="table-scroll"><table><thead><tr><th>기업·종목</th><th>대응 연구표기</th><th>종목 식별</th><th>가격일</th><th>사업 연결</th></tr></thead><tbody>${c.companies.map(r=>`<tr data-universe-symbol="${E(r.symbol)}"><td>${E(r.name)} · ${E(r.symbol)}</td><td>${E(r.aliases.join(' · '))}</td><td><a href="${E(safe(r.identity.source))}" target="_blank" rel="noopener noreferrer">${E(r.identity.method)} ↗</a> · ${E(r.identity.as_of)}</td><td>${E(r.price_date||'미확보')}</td><td>${r.business_relations}</td></tr>`).join('')}</tbody></table></div><p>미연결 표기 ${c.unresolved.map(r=>E(r.alias)+' ('+E(r.reason)+')').join(' · ')}</p></details>`;
 }
 function graphSVG(g,st={}){
  ...
  const nodes=g.nodes.filter(n=>(kind==='all'||n.kind===kind)&&(!q||(n.name+' '+(n.symbol||'')+' '+n.role+' '+(n.aliases||[]).join(' ')).toLocaleLowerCase().includes(q))),ids=new Set(nodes.map(n=>n.id));
 ...
 function bind(container,d,navigate,selection={}){
  ...
  container.querySelectorAll('[data-relation-view]').forEach(box=>{
   ...
   if(mode==='relationlab')body.innerHTML+=universeHTML(g)+insights(g);
