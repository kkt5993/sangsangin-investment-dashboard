...
(function(root){
 ...
 function hologram(c,yaw=.6,zoom=1,tilt=.48){
  const project=(x,y,z)=>project3d(x,y,z,yaw,tilt,zoom);
  ...
  rows.forEach((r,i)=>{const p=P(r),v=r.z.volatility,color=finite(r.z.herding)?`hsl(${210-(clamp(r.z.herding)+3)*30} 58% 46%)`:'#9ea5ab',radius=finite(v)?3+(clamp(v)+3):3;b+=`<circle data-holo-date="${r.date}" cx="${p[0]}" cy="${p[1]}" r="${i===rows.length-1?10:radius}" fill="${paint.fills[i]}" opacity="${.3+.7*i/Math.max(1,rows.length-1)}"><title>${r.date} · ${c.axes.map(a=>a.name+' '+n(r.raw[a.key])+' '+a.unit).join(' · ')}</title></circle>`;});
 ...
 function moeFan(c){
  const rows=c.history;if(!rows?.length)return empty();const W=700,H=260,L=48,R=74,T=24,B=32,F=70;
  ...
  for(const [key,label,opacity] of [['1.96','95',.13],['1','68',.27]]){const band=c.bands[key];if(band&&finite(last[1]))b+=`<polygon data-moe-band="${label}" points="${lx},${Y(last[1])} ${xf},${Y(band[1])} ${xf},${Y(band[0])}" fill="#5198b8" opacity="${opacity}"/>`;}
  ...
  b+=`<path data-moe-history="1" d="${path}" stroke="#388baa" stroke-width="2" fill="none"/>`;
 ...
 function moeTopology(cards){
  const W=980,H=390,ix=105,ex=400,gx=660,ox=855,names=cards[0]?.experts.map(r=>r.name)||[];if(!names.length)return empty();let b='';
  const weight=n=>cards.reduce((sum,c)=>sum+c.experts.find(r=>r.name===n).weight,0)/cards.length;
  names.forEach((name,i)=>{const y=35+i*35,w=weight(name);for(const iy of [125,260])b+=`<line x1="${ix+45}" x2="${ex-85}" y1="${iy}" y2="${y}" stroke="#b7c9d5" opacity=".55"/>`;b+=`<line data-moe-edge="${E(name)}" x1="${ex+85}" x2="${gx-45}" y1="${y}" y2="195" stroke="#4c8ca6" stroke-width="${.6+w/8}" opacity=".65"/><rect data-moe-expert="${E(name)}" x="${ex-85}" y="${y-13}" width="170" height="26" rx="6" fill="#edf3f8" stroke="#a6bbca"/><text x="${ex}" y="${y+4}" text-anchor="middle" class="axis">${E(name)} ${n(w)}%</text>`;});
  cards.forEach((c,i)=>{const y=cards.length===1?195:55+i*280/(cards.length-1);b+=`<line x1="${gx+45}" x2="${ox-65}" y1="195" y2="${y}" stroke="#9ab9ac"/><rect data-moe-output="${E(c.symbol)}" x="${ox-65}" y="${y-16}" width="130" height="32" rx="5" fill="#e3efe7"/><text x="${ox}" y="${y+4}" text-anchor="middle" class="axis">${E(c.name)}</text>`;});
  b+=`<rect x="${gx-45}" y="172" width="90" height="46" rx="7" fill="#f5e7c6"/><text x="${gx}" y="199" text-anchor="middle" class="axis">GATE</text>`;
 ...
 function bindCamera(canvas,options){
  ...
  let pending=null,disposed=false,moved=false,panX=0,panY=0,pinch=null,center=null,observer=null,tip=null;const contacts=new Map();
  ...
  const reset=()=>{if(pending!==null)root.cancelAnimationFrame?.(pending);pending=null;contacts.clear();pinch=null;center=null;options.start?.();panX=panY=0;options.reset();applyPan();canvas.scrollLeft=0;if(tip)tip.hidden=true;};
  ...
  canvas.addEventListener('pointerdown',e=>{if(e.button!==undefined&&![0,1,2].includes(e.button))return;canvas.focus?.({preventScroll:true});options.start?.();if(tip)tip.hidden=true;canvas.classList?.add('scene-engaged');contacts.set(e.pointerId??0,{x:e.clientX,y:e.clientY||0,startX:e.clientX,startY:e.clientY||0,pan:e.shiftKey||e.button===1||e.button===2});moved=false;const g=gesture();pinch=g?.distance;center=g;});
  canvas.addEventListener('pointermove',e=>{const id=e.pointerId??0,previous=contacts.get(id);if(!previous){if(tip){const target=e.target.closest?.('[data-state-date],[data-worm-symbol],[data-point],[data-node],[data-country],[data-holo-date],[data-surface-cell]'),text=target?.querySelector('title')?.textContent;if(text){tip.textContent=text;tip.hidden=false;const r=canvas.parentNode.getBoundingClientRect();tip.style.left=Math.max(12,Math.min(e.clientX-r.left+16,r.width-Math.min(300,r.width-24)-12))+'px';tip.style.top=Math.max(12,e.clientY-r.top-70)+'px';}else tip.hidden=true;}return;}
   ...
   if(Math.hypot(x-previous.startX,y-previous.startY)<4&&!moved)return;moved=true;canvas.setPointerCapture?.(id);canvas.classList?.add('scene-dragging');if(previous.pan){pan(x-previous.x,y-previous.y);return;}update({yaw:state.yaw+(x-previous.x)*.008,pitch:state.pitch+(y-previous.y)*.006});
  });
  const end=e=>{contacts.delete(e.pointerId??0);const g=gesture();pinch=g?.distance;center=g;if(!contacts.size)canvas.classList?.remove('scene-dragging');};
  ...
  canvas.addEventListener('blur',()=>{canvas.classList?.remove('scene-engaged');if(tip)tip.hidden=true;});
  canvas.addEventListener('contextmenu',e=>e.preventDefault());
  canvas.addEventListener('click',e=>{if(moved){e.preventDefault();e.stopImmediatePropagation();}},{capture:true});
  canvas.addEventListener('dblclick',reset);
  canvas.addEventListener('wheel',e=>{if(options.wheelRequiresCtrl!==false&&!e.ctrlKey&&canvas.ownerDocument?.activeElement!==canvas)return;e.preventDefault();options.start?.();if(tip)tip.hidden=true;update({zoom:options.read().zoom*Math.exp(-e.deltaY*.0015)});},{passive:false});
  canvas.addEventListener('keydown',e=>{if(!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','+','=','-','Home','Escape'].includes(e.key))return;if(e.target!==canvas&&e.key!=='Home')return;e.preventDefault();options.start?.();const v=options.read();if(e.key==='Escape'){canvas.blur?.();return;}if(e.key==='Home')reset();else if(['+','=','-'].includes(e.key))update({zoom:v.zoom+(e.key==='-'?-.05:.05)});else if(e.shiftKey)pan(e.key==='ArrowLeft'?-20:e.key==='ArrowRight'?20:0,e.key==='ArrowUp'?-20:e.key==='ArrowDown'?20:0);else update({yaw:v.yaw+(e.key==='ArrowLeft'?-.1:e.key==='ArrowRight'?.1:0),pitch:v.pitch+(e.key==='ArrowDown'?-.08:e.key==='ArrowUp'?.08:0)});});
 ...
 function orbPaint(palette,key){let hash=0;for(const c of String(key))hash=(Math.imul(hash,31)+c.charCodeAt(0))>>>0;const ids=palette.map((_,i)=>'orb-'+hash+'-'+i);return {fills:ids.map(id=>'url(#'+id+')'),defs:'<defs>'+palette.map((c,i)=>`<radialGradient id="${ids[i]}" cx="32%" cy="24%" r="82%"><stop offset="0" stop-color="#ffffff"/><stop offset=".18" stop-color="#f5fbff"/><stop offset=".38" stop-color="${E(c)}" stop-opacity=".55"/><stop offset=".7" stop-color="${E(c)}"/><stop offset=".88" stop-color="#21334f" stop-opacity=".85"/><stop offset="1" stop-color="#e5f1ff"/></radialGradient>`).join('')+'</defs>'};}
