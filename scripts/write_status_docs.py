...
PARITY={
 'overview':('전체 탭 요약·카드·탐색','26탭 상태, 데이터 수, 바로가기','원본의 개인 서술·Tesseract·crowding 보조 엔진은 제외'),
 'glance':('4단계 프로세스·구조·출처','현재 PC 수집→Python→JSON→Pages 흐름·소스·품질','원본 Mac 서버 운영 일정은 팀 일정으로 등록하지 않음'),
...
status_lines=['# 탭별 구현 현황','',f"가격 기준 {meta['as_of']} · 공개 탭 26개 · 분석/콘텐츠 연결 **{meta['implemented']}개** · Overview/At a Glance 2개 · ROSENBACH 1개 대기.",'',
...
for m in modules:
    ...
    first,rest=old.split('\n',1);block=['','<!-- implementation:start -->','## 현재 팀 구현 · 2026-09-09','',f'**{label}** · 가격 기준 {meta["as_of"]}. {scope}.','']
    if key in CODE:
...
print('Updated 26 module guides, implementation status and chart parity.')
