...
(function(root){
 ...
 function graphSVG(g,st={}){
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 560" class="relation-svg" role="img" aria-label="사업 관계망 3차원 투영. 객체 선택, 드래그 회전, 휠 확대."><rect width="900" height="560" rx="14" fill="#10101c"/>${paint.defs}${edges.map(edge).join('')}${[...nodes].sort((a,b)=>pos.get(b.id).z-pos.get(a.id).z).map(n=>{const p=pos.get(n.id),label=labels.get(n.id),on=n.id===selected;return `<g data-rnode="${E(n.id)}" tabindex="0" role="button" aria-label="${E(n.name)} 관계 선택" transform="translate(${p.x.toFixed(2)} ${p.y.toFixed(2)})" opacity="${!selected||on||neighbors.has(n.id)?1:.56}"><circle r="${p.r.toFixed(2)}" fill="${paint.fills[Object.keys(colors).indexOf(n.kind)]}" stroke="${on?'#fff':'#172939'}" stroke-width="${on?3:1}"/><text x="${(label.x-p.x).toFixed(2)}" y="${(label.y-p.y).toFixed(2)}" text-anchor="middle" fill="#e0eaf4" font-size="11">${E(label.text)}</text><title>${E(n.role)}${n.date?' · '+E(n.date):''}</title></g>`;}).join('')}<text x="18" y="540" fill="#b6c5d1" font-size="12">표시 ${nodes.length}객체 · ${edges.length}관계 · 점선: 가격 상관 · 드래그 회전 / 휠 확대</text></svg>`;
 }
