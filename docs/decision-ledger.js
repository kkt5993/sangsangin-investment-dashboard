...
(function(root){
 ...
 function card(r,entities){const it=intelligence(r),exists=entities.some(e=>e.id===r.object_id);return `<article class="panel ledger-card"><div class="title-row"><h3>${E(r.name)}</h3><span class="count-badge">${E(r.direction||'방향 미지정')} · ${E(r.status)}</span></div><p>확신도 ${r.conviction===null?'미설정':E('●'.repeat(r.conviction)+'○'.repeat(5-r.conviction))} · ${E(r.horizon||'시계 미설정')} · 비중 ${N(r.size)}%</p><p class="note-body">${E(r.thesis)}</p><dl><dt>촉매</dt><dd>${E(r.catalyst||'미기록')}</dd><dt>무효화 조건</dt><dd>${E(r.invalidate||'미기록')}</dd></dl><div class="tags">${it.checks.map(([k,v])=>`<span>${v?'✓':'○'} ${E(k)}</span>`).join('')}</div>${it.rr!==null||it.ev!==null?`<p>입력 가정 손익비 ${N(it.rr)}배 · 기대값 ${N(it.ev)}% · 이익 ${N(r.upside)}% / 손실 ${N(r.downside)}% / 확률 ${N(r.probability)}%</p>`:''}<small>생성 ${E(stamp(r.created_at))}<br>수정 ${E(stamp(r.updated_at))}${r.closed_at?'<br>청산 기록 '+E(stamp(r.closed_at)):''}</small><div class="note-actions">${r.deleted_at?button('restore','휴지통에서 복원',r.id):button('edit','수정',r.id)+(r.status==='제안'?button('activate','실행 상태로 기록',r.id):'')+(r.status==='청산'?button('reopen','실행 재개 기록',r.id):button('close','청산 상태로 기록',r.id))+(exists?button('entity','Entity 360',r.id):'')+button('delete','휴지통으로',r.id)}</div><details><summary>변경 이력 · 최근 ${r.history.length}건</summary><ul>${r.history.map(a=>`<li>${E(stamp(a.at))} · ${E(a.action)}</li>`).join('')}</ul></details></article>`;}
 const ui={filter:'all',q:'',draft:null,entity:'',kr:-10,us:-10};
 function bind(container,d,navigate,selection={}){
  const entities=d.sections.find(s=>s.type==='entities')?.entities||[];
  ...
  container.querySelectorAll('[data-decisions]').forEach(box=>{
   ...
   const repaint=()=>{
    ...
    body.querySelectorAll('[data-decision-action]').forEach(b=>b.addEventListener('click',()=>{try{
     ...
     else if(act==='entity'){navigate('Entity 360',{entity:vault.entries().find(r=>r.id===id).object_id});return;}
