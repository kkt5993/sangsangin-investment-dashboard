...
const ctx=vm.createContext({window:{},console,fetch:async url=>{assert.equal(url,'data/coastlines.json');return {ok:true,json:async()=>JSON.parse(read(url.replace('data/','docs/data/')))};}});
...
module.exports=(async()=>{
 ...
 const base={lon:0,lat:20,zoom:1,size:1,selected:'',mode:'rgb'},site=s.sites.find(r=>r.scene),focused=V.focus(base,site);assert.equal(focused.zoom,14);assert.equal(focused.lon,site.lon);assert.equal(V.camera(focused,0,0,100).zoom,17);assert.equal(V.camera(base,0,0,-10).zoom,1);
 ...
 await box.querySelector('[data-sat-size]').fire('click');assert(surface.innerHTML.includes('height="760"'));await box.querySelector('[data-sat-reset]').fire('click');assert(surface.innerHTML.includes('확대 1'));
