...
(function(root){
 'use strict';
 const MAX_RECORDS=500,MAX_FILE=8*1024*1024,MAX_FILES=64*1024*1024,MAX_META=2*1024*1024,MAX_IMPORT=90*1024*1024;
 ...
 const empty=()=>({version:1,revision:'',entries:[],files:[]});
 function validate(book){
  if(!book||book.version!==1||typeof book.revision!=='string'||!Array.isArray(book.entries)||book.entries.length>MAX_RECORDS||!Array.isArray(book.files))fail('노트북 형식·500건 한도를 확인하세요.');
  const b={version:1,revision:book.revision,entries:book.entries.map(entry),files:book.files.map(f=>{if(!f||!/^[a-f0-9]{64}$/.test(f.id)||!['application/pdf','image/png','image/jpeg','image/webp'].includes(f.mime)||!Number.isInteger(f.size)||f.size<1||f.size>MAX_FILE)fail('첨부 메타데이터 오류');return {id:f.id,mime:f.mime,size:f.size};})};
 ...
 function open(indexedDB=root.indexedDB,name='sangsangin-research-v1'){
  return new Promise((resolve,reject)=>{if(!indexedDB){reject(Error('이 브라우저에서 로컬 자료 저장을 사용할 수 없습니다.'));return;}const req=indexedDB.open(name,1);let abandoned=false;
   req.onupgradeneeded=()=>{const db=req.result;db.createObjectStore('meta');db.createObjectStore('files');};req.onerror=()=>reject(req.error);req.onblocked=()=>{abandoned=true;reject(Error('다른 창의 이전 자료 연결을 닫고 새로고침하세요.'));};
 ...
 async function encode(book,repo,module){const entries=book.entries.filter(r=>r.module===module),ids=new Set(entries.flatMap(r=>r.attachments.map(f=>f.id))),files=[];
  ...
  return JSON.stringify({format:'sangsangin-research',version:1,revision:book.revision,entries,files});
 }
 ...
 root.ResearchStore={open,create,entry,validate,edit,trash,purge,merge,legacy,prepare,encode,decode,empty,MAX_FILE,MAX_FILES,MAX_IMPORT,KINDS};
