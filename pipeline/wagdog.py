...
def wagdog_views(d,obj):
    ...
    obj['method_note']+=' 파생 Wag-the-Dog는 행사가 가로축·GEX 세로 막대·현물/플립/기대폭과 콜 위/풋 아래 OI 구조입니다. OI의 상하 배치는 매수·매도 방향을 뜻하지 않습니다. 30일 기대폭은30일에 가장 가까운 수집 만기의 같은 ATM 행사가 콜/풋 IV 평균×√(30/365.25) 근사입니다. 감마는IV 유효 계약만, OI는IV 결측도 포함합니다. 수집 시각의 현물과 만기를 그대로 사용하며 현재 시세로 재평가하지 않습니다. 수집 범위 메타데이터(예: DTE, 행사가 밴드, 만기 제한)는 `옵션 관측 범위`에서 표시합니다.'
