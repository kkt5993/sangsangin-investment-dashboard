...
def build(d):
    ...
    for func in [etf_monitor,multiasset,dynamics,regimes,risk,weekend,geoecon]:d.export(func(d))
    d.export(watch(d,rank));d.export(quant(d))
    financial=financial_rows(d);consensus,casof=local_growth(d)
    for obj in [earnings(d,financial,consensus),growth(d,financial,consensus,casof),discovery(d,financial,rank),strategies(d,financial)]+libraries(d)+networks(d,financial,rank):d.export(obj)
    status={}
    for file in (ROOT/'docs/data').glob('*.json'):
        ...
        status[a['module']]=dict(status=a['status'],as_of=a['as_of'],bytes=file.stat().st_size,sections=len(a.get('sections',[])),missing=a.get('missing',[]))
    status['overview']=dict(status='operational',as_of=d.as_of);status['glance']=dict(status='operational',as_of=d.as_of)
    status['rosenbach']=dict(status='blocked',as_of=d.as_of,missing=['소유자 전용 데이터·KSI 산식·인증 서버가 공개되지 않았습니다.'])
    overview=dict(as_of=d.as_of,price_series=len(d.frames),macro_series=len(d.macro),financial_companies=len(financial),consensus_as_of=casof,price_quality=d.correction_meta,universes={k:{x:v for x,v in r.items() if x in ['available','expected','membership_as_of','source']} for k,r in rank.items()},
