...
(function(root){
 ...
 function entry(a){
  ...
  if(!Array.isArray(a.attachments))fail('첨부 목록 형식을 확인하세요.');
  ...
  if(a.extractions!==undefined&&(!Array.isArray(a.extractions)))fail('PDF 추출 연결 오류');
 ...
 function validate(book){
  if(!book||![1,2].includes(book.version)||typeof book.revision!=='string'||!Array.isArray(book.entries)||!Array.isArray(book.files))fail('노트북 형식을 확인하세요.');
  const b={version:2,revision:book.revision,entries:book.entries.map(entry),files:book.files.map(f=>{if(!f||!/^[a-f0-9]{64}$/.test(f.id)||!['application/pdf','image/png','image/jpeg','image/webp'].includes(f.mime)||!Number.isInteger(f.size)||f.size<1)fail('첨부 메타데이터 오류');return {id:f.id,mime:f.mime,size:f.size};})};
 ...
 async function legacy(raw,module){const list=JSON.parse(raw);if(!Array.isArray(list))fail('이전 메모 형식을 확인하세요.');const entries=[];for(const a of list){const hash=await root.crypto.subtle.digest('SHA-256',new TextEncoder().encode(JSON.stringify([a.title,a.body,a.date]))),id='legacy-'+module+'-'+Array.from(new Uint8Array(hash),v=>v.toString(16).padStart(2,'0')).join('');if(entries.some(r=>r.id===id))continue;entries.push(create(module,{id,title:str(a.title,160,true),core:str(a.body,12000,true),date:time(a.date)?a.date.slice(0,10):now().slice(0,10),history:[{at:now(),action:'이전 메모 가져오기'}]}));}return {...empty(),entries};}
 ...
 async function decode(raw){const a=JSON.parse(raw);if(a?.format!=='sangsangin-research')fail('리서치 백업 파일이 아닙니다.');const book=validate(a),prepared=[];
  for(const f of a.files){if(typeof f.data!=='string'||!/^[A-Za-z0-9+/]*={0,2}$/.test(f.data))fail('첨부 인코딩 오류');const binary=atob(f.data),buffer=Uint8Array.from(binary,c=>c.charCodeAt(0)),p=await prepare(new Blob([buffer]),'backup');if(p.meta.id!==f.id||p.meta.size!==f.size||p.meta.mime!==f.mime)fail('첨부 파일의 해시·크기·형식이 일치하지 않습니다.');prepared.push(p);}
  return {book,prepared};
 }
 root.ResearchStore={open,create,entry,validate,extraction,edit,trash,purge,merge,legacy,prepare,encode,decode,empty,KINDS};
