...
async function checks(){
 ...
 const html=V.view(data);assert(!html.includes('type="range"'),'direct manipulation replaces camera sliders');assert(V.hologram(p).includes('data-glass-plate'),'glass spatial plane');assert(V.radar(p).includes('radar-glass-'),'date-specific glass layers');assert(!/NaN|undefined|Infinity/.test(html));const unsafe=structuredClone(data);unsafe.leaders.stocks[0].name='<script>alert(1)</script>';assert(!V.view(unsafe).includes('<script>'));assert(V.view(unsafe).includes('&lt;script&gt;'));
