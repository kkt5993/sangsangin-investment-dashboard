...
(function(root){
 ...
 function form(){return `<section class="notebook-pdf"><h4>PDF 본문 읽기</h4><p class="quiet">첨부를 선택하면 이 브라우저에서 읽습니다. 최대300쪽·20만자. 자동 모드는 파서로 읽고 본문이 부족하거나 깨진 페이지만 OCR로 읽습니다. OCR은 한 번에30쪽까지이며 다시 읽으면 이어서 처리합니다. OCR 숫자·표는 원문과 대조하세요. 본문·페이지 번호는 기록을 저장할 때 함께 보관합니다.</p><label>읽을 PDF<select data-nb-pdf-select><option value="">PDF를 첨부하세요</option></select></label><label>처리 방식<select data-nb-pdf-mode><option value="auto">자동 · 파서 우선, 필요한 페이지만 OCR</option><option value="text">파서만 · 가장 빠름</option><option value="ocr">전체 OCR · 이미지 속 본문 확인</option></select></label><label>OCR 언어<select data-nb-pdf-language><option value="kor+eng">한국어 + 영어</option><option value="eng">영어</option></select></label><label>문서 배치<select data-nb-pdf-layout><option value="single">한 단 · 일반 본문</option><option value="columns">여러 단 · 복잡한 배치</option></select></label><div class="note-actions"><button type="button" data-nb-pdf-extract>선택 PDF 본문 읽기</button><button type="button" data-nb-pdf-cancel disabled>읽기 취소</button></div><output data-nb-pdf-status role="status"></output><div data-nb-pdf-result hidden><label>페이지<select data-nb-pdf-page></select></label><label>근거로 가져올 원문 (필요한 부분만 남기세요)<textarea data-nb-pdf-quote rows="9" maxlength="12000"></textarea></label><div class="note-actions"><button type="button" data-nb-pdf-evidence>근거에 원문·페이지 추가</button><button type="button" data-nb-pdf-download>추출 본문 TXT ↓</button></div></div></section>`;}
 function mount(el,o){
  ...
  const show=()=>{const {data}=current();result.hidden=!data;if(!data){status.textContent='PDF를 선택하고 본문 읽기를 실행하세요.';pages.innerHTML='';quote.value='';return;}status.textContent=P.status(data)+' · 저장 버튼을 눌러 본문과 첨부를 보관하세요.';const old=lastID===data.file_id?pages.value:'1';lastID=data.file_id;pages.innerHTML=data.pages.map(p=>`<option value="${p.number}">PDF ${p.number}쪽 · 표기 ${E(p.label)} · ${E(P.method(p))}${p.confidence!==null&&p.confidence!==undefined?' · 인식 점수 '+p.confidence.toFixed(0):''}${p.cut?' · 일부':''}${p.text?'':' · 텍스트 없음'}</option>`).join('');pages.value=data.pages.some(p=>String(p.number)===old)?old:'1';pageText();};
  ...
  q('[data-nb-pdf-extract]').addEventListener('click',()=>o.run(async()=>{
   ...
   try{const blob=f.blob||await o.repo.blob(f.id),previous=current().data,a=await (o.extract||P.extract)(blob,{signal:controller.signal,mode:q('[data-nb-pdf-mode]').value,language:q('[data-nb-pdf-language]').value,layout:q('[data-nb-pdf-layout]').value,previous,onProgress:p=>{if(!disposed)status.textContent=p.phase==='ocr-load'?'OCR 처리기 준비 중 · 첫 실행에는 모델을 내려받습니다.':`PDF ${p.page}/${p.total}쪽 ${p.phase==='ocr'?'OCR 인식':'읽는'} 중 · ${p.characters.toLocaleString()}자`;}});
