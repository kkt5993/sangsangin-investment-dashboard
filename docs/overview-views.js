...
(function(root){
 ...
 function hologram(p,yaw=.6){
  const W=750,H=450,roles=Object.fromEntries(p.axes.map(a=>[a.role,a.key]));
  const P=(x,z,y)=>{const rx=x*Math.cos(yaw)-z*Math.sin(yaw),rz=x*Math.sin(yaw)+z*Math.cos(yaw);return [W/2+rx*178,H*.51+rz*71-y*104];};
  ...
  p.rows.forEach((row,i)=>{const pt=point(row);if(!pt)return;const size=row.z[roles.size],halo=roles.halo?row.z[roles.halo]:null,r=valid(size)?3+clip((size+1.5)/3,0,1)*6:4,last=i===p.rows.length-1;
   ...
   b+=`<circle data-state-date="${row.date}" cx="${pt[0]}" cy="${pt[1]}" r="${r}" fill="${color(row.z[roles.color])}" stroke="${last?'#b88a11':'#f5f8fa'}" stroke-width="${last?3:1}" opacity="${last?1:.35+.6*i/p.rows.length}"><title>${E(row.date)} · ${p.axes.map(a=>E(a.name)+' '+N(row.raw[a.key])+' '+E(a.unit)+' (z '+N(row.z[a.key])+')').join(' · ')}</title></circle>`;
 ...
 function radar(p){
  const W=400,H=420,cx=200,base=322,gap=54,R=111,n=p.axes.length,angle=i=>-Math.PI/2+i*2*Math.PI/n;
  const P=(i,z,level)=>{const r=Math.max(4,(clip(z,-2,2)+2)/4*R);return [cx+Math.cos(angle(i))*r,base-level*gap+Math.sin(angle(i))*r*.46];};
 ...
 function wormhole(l,phase=.3){
  if(!l.stocks.length)return '<p>주도주 입력 부족</p>';const W=440,H=440,cx=220,cy=210,R=180,flat=.47,logs=l.stocks.map(s=>Math.log(s.cap_usd_bn)),lo=Math.min(...logs),hi=Math.max(...logs),center=u=>cy-R*.42*(1-u);let b='';
  ...
  l.stocks.map((s,i)=>({s,i,depth:1-(Math.log(s.cap_usd_bn)-lo)/(hi-lo||1)})).sort((a,b)=>a.depth-b.depth).forEach(({s,i,depth})=>{const u=.18+.82*depth,r=R*u**1.12,a=phase+i*2*Math.PI/l.stocks.length,x=cx+Math.cos(a)*r,y=center(u)+Math.sin(a)*r*flat,radius=(5+clip(s.growth/250,0,1)*13)*(.5+.62*u);
   let labelY=y-radius-9;const width=s.symbol.replace('.KS','').length*8;for(let j=0;j<10&&occupied.some(r=>Math.abs(r.y-labelY)<17&&Math.abs(r.x-x)<(r.width+width)/2+5);j++)labelY-=18;occupied.push({x,y:labelY,width});
   b+=`<g data-worm-symbol="${E(s.symbol)}" tabindex="0" role="button" aria-label="${E(s.name)} 관측 보기"><line x1="${x}" y1="${y}" x2="${cx}" y2="${center(0)}" stroke="#b3c4d4"/><circle cx="${x}" cy="${y}" r="${radius}" fill="${color(s.momentum/55)}" stroke="#f9fafc" stroke-width="2"/><text x="${x}" y="${labelY}" text-anchor="middle" font-size="13" fill="#233e54">${E(s.symbol.replace('.KS',''))}</text><title>${E(s.name)} · 시총 ${N(s.cap_usd_bn)} USD bn · 모멘텀 ${N(s.momentum)}% · 분기 NI 성장 ${N(s.growth)}% · 주도력 ${N(s.score)}</title></g>`;
  });
 ...
 function bind(box,d){box.querySelectorAll('[data-coordinate]').forEach(el=>{
  ...
  el.querySelector('[data-state-reset]').addEventListener('click',()=>{stop();yaw=.6;slider.value='60';draw();});
  const graph=el.querySelector('[data-state-holo]');graph.addEventListener('pointerdown',e=>{stop();drag=e.clientX;graph.setPointerCapture?.(e.pointerId);});graph.addEventListener('pointermove',e=>{if(drag===null)return;yaw+=(e.clientX-drag)*.009;drag=e.clientX;slider.value=String(yaw*100);draw();});graph.addEventListener('pointerup',()=>{drag=null;});graph.addEventListener('pointercancel',()=>{drag=null;});
  disposers.push(()=>{disposed=true;stop();});draw();
 });}
